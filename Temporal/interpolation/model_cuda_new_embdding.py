...
class T_RED_GNN(nn.Module):
    def __init__(self, params):
        ...
        self.time_embed = PeriodicEmbeddings(1, self.hidden_dim, lite=False)
    ...
    def forward(self, batch, mode='train'):
        ...
        for i in range(self.n_layer):
            ...
            embed_rel = hidden_embedding[node_new_index[select_relation[:,0], select_relation[:,1]]-1, :] + self.rela_embed_layer[i](select_relation[:,2]) + self.time_embed(relative_time_index.reshape(-1, 1)).squeeze(1)
        ...
        result = self.linear_classifier(hidden_embedding).reshape(-1) #+ self.query_relation_linear(self.score_embed_layer(query_rel)).reshape(-1)
