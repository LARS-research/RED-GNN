...
class T_RED_GNN(nn.Module):
    def __init__(self, params, hps):
        ...
        self.attn_dim = hps.attn_dim
        self.act = hps.act
        self.n_layer = hps.n_layer
        self.rela_embed = nn.Embedding(self.n_rel+1, self.hidden_dim)
        self.attention_1 = nn.Linear(self.hidden_dim * 3, self.attn_dim, bias=False)
        self.attention_2 = nn.Linear(self.attn_dim, 1, bias=False)
        ...
        self.time_embed = nn.Embedding(365, self.hidden_dim)
    ...
    def forward(self, batch):
        ...
        dataset = np.delete(self.dataset, batch['example_idx'].cpu().numpy(), axis=0)
        ...
        for i in range(self.n_layer):
            ...
            embed_rel = hidden_embedding[node_new_index[select_relation[:,0], select_relation[:,1]]-1, :] + self.rela_embed(select_relation[:,2]) + self.time_embed(torch.abs(relative_time_index))
            ...
            if self.act == 'sigmoid':
                hidden_embedding = torch.sigmoid(hidden_embedding)
            elif self.act == 'relu':
