...
class T_RED_GNN_wo_tau(nn.Module):
    def __init__(self, params):
        super(T_RED_GNN_wo_tau, self).__init__()
...
class T_RED_GNN_W(nn.Module):
    def __init__(self, params):
        super(T_RED_GNN_W, self).__init__()
        self.n_rel = len(params.relation_vocab)
        self.device = params.device
        self.n_ent = len(params.entity_vocab)
        self.n_time = len(params.time_vocab)
        ...
        self.past_bias = nn.Parameter(torch.zeros(1, self.hidden_dim))
        self.now_bias = nn.Parameter(torch.zeros(1, self.hidden_dim))
        self.future_bias = nn.Parameter(torch.zeros(1, self.hidden_dim))
    ...
    def forward(self, batch, mode='train'):
        ...
        for i in range(self.n_layer):
            ...
            transformed_embed = torch.zeros_like(embed_rel, device=self.device)
            transformed_embed[relative_time_index > 0] = embed_rel[relative_time_index > 0] + self.future_bias
            transformed_embed[relative_time_index == 0] = embed_rel[relative_time_index == 0] + self.now_bias
            transformed_embed[relative_time_index < 0] = embed_rel[relative_time_index < 0] + self.past_bias
...
class T_RED_GNN_wo_Attn(nn.Module):
    def __init__(self, params):
        super(T_RED_GNN_wo_Attn, self).__init__()
        self.n_rel = len(params.relation_vocab)
        self.device = params.device
        self.n_ent = len(params.entity_vocab)
        self.n_time = len(params.time_vocab)
        ...
        self.n_layer = params.n_layer
        self.rela_embed_layer = nn.ModuleList([nn.Embedding(self.n_rel+1, self.hidden_dim) for _ in range(self.n_layer)])
        self.score_embed_layer = nn.Embedding(self.n_rel+1, self.hidden_dim)
        self.attention_1_layer = nn.ModuleList([nn.Linear(self.hidden_dim * 3, self.attn_dim, bias=False) for _ in range(self.n_layer)])
        self.attention_2_layer = nn.ModuleList([nn.Linear(self.attn_dim, 1, bias=False) for _ in range(self.n_layer)])
        self.linear_classifier = nn.Linear(self.hidden_dim, 1)
        self.query_relation_linear = nn.Linear(self.hidden_dim, 1, bias=False)
    ...
    def forward(self, batch, mode='train'):
        ...
        # calculate the cover rate
        # print(f'Coverage rate is {100 * self.coverage_rate(batch, cur_entity)}%')
        result = self.linear_classifier(hidden_embedding).reshape(-1) + self.query_relation_linear(self.score_embed_layer(query_rel[cur_entity[:,0]])).reshape(-1)
        score_all = torch.zeros(batch_size, self.n_ent, device=self.device)
        score_all[cur_entity[:, 0], cur_entity[:, 1]] = result
        return score_all
