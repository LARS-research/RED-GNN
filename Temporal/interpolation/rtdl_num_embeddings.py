    ...
    **Examples**
    ...
    def __init__(
        ...
        *,
        ...
        # self.linear_neg: Union[nn.Linear, _NLinear]
        # self.linear_pos: Union[nn.Linear, _NLinear]
        if lite:
            # NOTE[DIFF]
            ...
            # self.linear_neg = nn.Linear(2 * n_frequencies, d_embedding)
            # self.linear_pos = nn.Linear(2 * n_frequencies, d_embedding)
        else:
            self.linear = _NLinear(n_features, 2 * n_frequencies, d_embedding)
            # self.linear_neg = _NLinear(n_features, 2 * n_frequencies, d_embedding)
            # self.linear_pos = _NLinear(n_features, 2 * n_frequencies, d_embedding)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        is_neg = (x < 0).unsqueeze(-1)  # [N, 1, 1]
        x = self.periodic(x)  # [N, 1, 96]
        x = self.linear(x)
        # neg_x = self.linear_neg(x) # [N, 1, 20]
        # pos_x = self.linear_pos(x)  # [N, 1, 20]

        # x = neg_x*is_neg + pos_x*(~is_neg)
