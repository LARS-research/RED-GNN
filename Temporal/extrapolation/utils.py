...
import torch
# from model_cuda import T_RED_GNN, RED_GNN
# from model_cuda_aba import T_RED_GNN_W as T_RED_GNN
# from model_cuda_aba import T_RED_GNN_wo_tau as T_RED_GNN
# from model_cuda_aba import T_RED_GNN_wo_Attn as T_RED_GNN
from model_cuda_new_embedding import T_RED_GNN
...
class Data:
    def __init__(self, dataset=None, add_reverse_relation=False):
        ...
        if add_reverse_relation:
            ...
            self.valid_data_seen_entity = self.valid_data_seen_entity[:, :-1]
        ...
        if add_reverse_relation:
            ...
            self.test_data_seen_entity = self.test_data_seen_entity[:, :-1]
            self.test_data_unseen_entity = self.test_data_unseen_entity[:, :-1]
            
                                                              
        ind=np.argsort(self.train_data[:,-1])
        self.train_data = self.train_data[ind]

        ind=np.argsort(self.valid_data[:,-1])
        self.valid_data = self.valid_data[ind]

        ind=np.argsort(self.test_data[:,-1])
        self.test_data = self.test_data[ind]

        self.data = np.concatenate([self.train_data, self.valid_data, self.test_data], axis=0, dtype=np.int64)
...
def load_checkpoint(checkpoint_dir, device='cpu', args=None):
    ...
    params = Options
    params.n_layer = 3
    params.hidden_dim = 60
    params.attn_dim = 60
    params.act = 'leakyrelu'
    params.dropout = 0.1
    if os.path.isfile(checkpoint_dir):
        ...
        if 'yago' in args.dataset.lower():
            params.time_granularity = 1
            time_granularity = 1
        elif 'icews' in args.dataset.lower():
            params.time_granularity = 24
            time_granularity = 24
        else:
            raise ValueError
...
def get_time_offset_list(data, time_granularity=24):
    max_time = max(data[:, 3] // time_granularity)
    offset_list = np.zeros(max_time+2, dtype=np.int32)
    print(data[0])
    for idx, (_, _, _, t) in enumerate(data):
        cur_t = t // time_granularity
        offset_list[cur_t+1] = idx
    return offset_list

# def get_time_offset_list(data):
#     offset_list = []
#     pre_t = -1
#     for idx, (_, _, _, t) in enumerate(data):
#         cur_t = t // 24
#         if pre_t != cur_t:
#             offset_list.append(idx)
#             pre_t = cur_t
#     return offset_list
