...
class T_RED_GNN(nn.Module):
    ...
    def forward(self, X):
        ...
        adj_batch_list = [coo_matrix(
                (
                    np.ones(relation_t[i].shape[0]), 
                    (np.arange(relation_t[i].shape[0]), relation_t[i][:,1])
                ), shape=(relation_t[i].shape[0], self.n_ent)
            ) for i in range(batch_size)]
        adj_batch_list = scipy.sparse.block_diag(adj_batch_list)
        ...
        for i in range(self.n_layer):
            ...
            select_one_hot = adj_batch_list @ batch_data_sp_vec
