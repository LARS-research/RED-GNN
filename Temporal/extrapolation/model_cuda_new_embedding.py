...
class T_RED_GNN(nn.Module):
    def __init__(self, params):
        ...
        self.time_granularity = params.time_granularity
        try:
            self.more_time_dim = params.more_time_dim
        except:
            self.more_time_dim = 1
        # self.more_time_dim = params.more_time_dim
        ...
        self.time_embed = PeriodicEmbeddings(1, self.hidden_dim*self.more_time_dim, n_frequencies= 48, lite=False)

        # liushuzhi add
        self.time_embed_absolute_query = PeriodicEmbeddings(1, self.hidden_dim*self.more_time_dim, n_frequencies= 48, lite=False)
        self.time_embed_absolute_graph = PeriodicEmbeddings(1, self.hidden_dim*self.more_time_dim, n_frequencies= 48, lite=False)
    ...
    def forward(self, X):
        ...
        cur_t = torch.from_numpy(X.ts // self.time_granularity).to(self.device)
        ...
        for b_idx in range(batch_size):
            # TOOD: WHY 120? how about 5*time_granularity?
            ...
            selfloop_rel = np.column_stack([np.arange(self.n_ent), np.ones(self.n_ent, dtype=np.int64)*self.n_rel_true, np.arange(self.n_ent), begin_time * self.time_granularity * np.ones(self.n_ent, dtype=np.int64)])
            data_batch = np.concatenate([selfloop_rel, data_batch], axis=0)
            n_rel_t = data_batch.shape[0]
            data_batch = np.concatenate([b_idx * np.ones((n_rel_t, 1), dtype=np.int64), data_batch], axis=1)  # this first element seems to indicate the batch index
        ...
        for i in range(self.n_layer):
            ...
            # get embedding
            relative_time_index = cur_t[select_relation[:, 0]] - torch.div(select_relation[:, 4], self.time_granularity, rounding_mode='floor')

            # liushuzhi add
            # absolute_time_query = cur_t[select_relation[:, 0]] - min(X.ts) // self.time_granularity
            # absolute_time_in_graph = torch.div(select_relation[:, 4], self.time_granularity, rounding_mode='floor') - min(X.ts) // self.time_granularity
            # relative_time_index = torch.div(select_relation[:, 4], self.time_granularity, rounding_mode='floor').type(torch.float32)

            # original version of time_embedding
            ...
            # time_embedding =self.time_embed(relative_time_index.reshape(-1, 1)).squeeze(1) + self.time_embed_absolute_query(absolute_time_query.reshape(-1, 1)).squeeze(1) + self.time_embed_absolute_graph(absolute_time_in_graph.reshape(-1, 1)).squeeze(1)
            # liushuzhi use the following
            time_embedding =self.time_embed(relative_time_index.reshape(-1, 1)).squeeze(1)  #+ self.time_embed_absolute_query(absolute_time_query.reshape(-1, 1)).squeeze(1) + self.time_embed_absolute_graph(absolute_time_in_graph.reshape(-1, 1)).squeeze(1)
            ...
            # for j in range(select_relation.shape[0]):
            #     attention_dict[select_relation[j, 4].item()//24].append(attention_score[j].item())
        ...
        # self.attention_vis.append(attention_dict)
                # self.attention_vis.append(attention_dict)
        with open('attention_vis_with_time0821.pickle', 'wb') as handle:
            pickle.dump(self.attention_vis, handle, protocol=pickle.HIGHEST_PROTOCOL)

        with open('attention_time_vis.pickle0821', 'wb') as handle:
            pickle.dump(self.attention_vis, handle, protocol=pickle.HIGHEST_PROTOCOL)
