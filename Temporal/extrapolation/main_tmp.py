...
def prepare_inputs(contents, dataset='train', start_time=0, tc=None):
    ...
    index_array = np.arange(contents_dataset.shape[0]).reshape(-1, 1)
    events = np.concatenate([contents_dataset, index_array], axis=1)
    if args.timer:
...
class SimpleCustomBatch:
    def __init__(self, data):
        ...
        # self.event_idx = np.arange(self.src_idx.shape[0])
...
if __name__ == "__main__":
    ...
    train_inputs = prepare_inputs(contents, start_time=0, tc=time_cost)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for batch_ndx, sample in enumerate(train_data_loader):
            ...
            score, (entity_att_score, entities) = model(sample, mode='train')
        ...
        if epoch % 1 == 0:
            ...
            writer.add_scalar('Hit1/Valid', np.mean(val_loss_list), epoch)
            print(
