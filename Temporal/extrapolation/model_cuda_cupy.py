...
class T_RED_GNN(nn.Module):
    ...
    def forward(self, X):
        ...
        with cp.cuda.Device(int(self.device[-1])):
            adj_batch = coo_matrix(
                    (
                        cp.ones(relation_all_t.shape[0]), 
                        (cp.arange(relation_all_t.shape[0]), cp.asarray(relation_all_t[:,1]+relation_all_t[:,0]*self.n_ent))
                    ), shape=(relation_all_t.shape[0], batch_size*self.n_ent)
                )

        for i in range(self.n_layer):
            with cp.cuda.Device(int(self.device[-1])):
                batch_data_sp_vec = coo_matrix((cp.ones(cur_entity.shape[0]), (cp.asarray(cur_entity[:,1]+cur_entity[:,0]*self.n_ent), cp.zeros(cur_entity.shape[0]))),shape=(batch_size*self.n_ent, 1))
                select_one_hot = adj_batch.dot(batch_data_sp_vec)
                select_idx = select_one_hot.nonzero()
