...
parser.add_argument('--more_time_dim', type=int, default=1, help='how many times time_dim are compared to hidden_dim')
args = parser.parse_args()

params = Options
params.n_layer = 3
params.hidden_dim = 30
params.attn_dim = 30
params.act = 'leakyrelu'
params.dropout = 0.1
params.device = 'cuda:{}'.format(args.device) if args.device >= 0 else 'cpu'
params.patience = 3
params.more_time_dim = args.more_time_dim
# args.batch_size = 20
# args.lr = 10**-2.69039
if not args.debug:
    import local_config
...
if __name__ == "__main__":
    ...
    if args.load_checkpoint is None:
        ...
        if 'yago' in args.dataset.lower():
            ...
            raise ValueError
        time_offset_list = get_time_offset_list(contents.data, time_granularity)
        ...
        optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=1*1e-3)
        ...
        checkpoint = torch.load(os.path.join(save_dir, 'Checkpoints', args.load_checkpoint), map_location=torch.device('cpu'))
    ...
    val_spt2o = contents.get_spt2o('test')
    train_inputs = prepare_inputs(contents, start_time=args.warm_start_time, tc=time_cost)
    val_inputs = prepare_inputs(contents, dataset='test', tc=time_cost)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        if epoch % 1 == 0:
            ...
            val_loss_list = []
            valid_cnt = 0
            pbar = tqdm(val_data_loader, desc=f'Epoch={epoch}', mininterval=60)
            for batch_ndx, sample in enumerate(pbar):
                model.eval()
                ...
                pbar.set_postfix({'HIT@1': val_running_hit1_sum / (batch_ndx + 1)})
            ...
            writer.add_scalar('Hit1/Valid', val_running_hit1_sum / valid_cnt, epoch)
            print(
