...
if __name__ == '__main__':
    ...
    opts.mem_file = os.path.join(results_dir, dataset + '_peak_mem.txt')
    ...
    # Write overall peak memory (across all epochs) to results/
    try:
        train_peak = model._train_mem_peak or {'cpu_rss_peak_mb': 0.0, 'cuda_alloc_peak_mb': 0.0, 'cuda_reserved_peak_mb': 0.0}
        infer_peak = model._infer_mem_peak or {'cpu_rss_peak_mb': 0.0, 'cuda_alloc_peak_mb': 0.0, 'cuda_reserved_peak_mb': 0.0}
        mem_line = (
            f"[OVERALL_PEAK_MEM_MB] "
            f"train_cpu:{train_peak['cpu_rss_peak_mb']:.1f} "
            f"train_cuda_alloc:{train_peak['cuda_alloc_peak_mb']:.1f} "
            f"train_cuda_rsv:{train_peak['cuda_reserved_peak_mb']:.1f} "
            f"inf_cpu:{infer_peak['cpu_rss_peak_mb']:.1f} "
            f"inf_cuda_alloc:{infer_peak['cuda_alloc_peak_mb']:.1f} "
            f"inf_cuda_rsv:{infer_peak['cuda_reserved_peak_mb']:.1f}\n"
        )
        print(mem_line.strip())
        with open(opts.mem_file, 'w') as f:
            f.write(mem_line)
    except Exception as e:
        print('Failed to write peak memory log:', e)
    print(best_str)
